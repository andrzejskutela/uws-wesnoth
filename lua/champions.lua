local find_conditional_buffs = function(pool_settings)
	...
	if pool_settings['has_ranged'] then
		...
		if pool_settings['has_magical_ranged'] then
			for k,v in ipairs({ 'A42','A43','A44','A61','B51','B51', 'C28','C37','C47','C64', }) do segregation_pool[#segregation_pool + 1] = v end
			...
			if pool_settings['has_multiple_str_ranged'] then
				for k,v in ipairs({ 'B62','B62','B62', }) do segregation_pool[#segregation_pool + 1] = v end
			end
		end

		if pool_settings['has_ranged_slow'] then
			for k,v in ipairs({ 'C52','C52','C64',  }) do segregation_pool[#segregation_pool + 1] = v end
		end
	elseif pool_settings['has_melee'] then
		for k,v in ipairs({ 'A4','A54','A54','A62','B20','B34','C48','C48', }) do segregation_pool[#segregation_pool + 1] = v end

		if not pool_settings['has_magical_melee'] and pool_settings['has_multiple_str_melee'] then
			for k,v in ipairs({ 'B61','B61','B61', }) do segregation_pool[#segregation_pool + 1] = v end
		end
	end
	
	if pool_settings['has_melee'] then
		for k,v in ipairs({ 'A2','A6','A21','A28','A28','A33','A34','A41','A45','A47','A49','A54','A60','A60','A62', 'B2','B4','B6','B9','B27','B30','B33','B41', 'C7','C10','C14','C20','C25','C39', }) do segregation_pool[#segregation_pool + 1] = v end
	...
	if pool_settings['is_high_level'] then
		for k,v in ipairs({ 'B36','B36','B36','A37','A37','B37','B40','B64','C49','C49','C64', }) do segregation_pool[#segregation_pool + 1] = v end

		if pool_settings['is_chaotic'] and not pool_settings['has_dark_pact'] then
			for k,v in ipairs({ 'A55','A55','A55','A55','B64' }) do segregation_pool[#segregation_pool + 1] = v end
		end

		if pool_settings['is_chaotic'] and not pool_settings['has_protected_by_darkness'] then
			for k,v in ipairs({ 'B64','C61','C61','C61' }) do segregation_pool[#segregation_pool + 1] = v end
		end
	...
	if pool_settings['is_chaotic'] then
		for k,v in ipairs({ 'C56','C56','A57','A57','A64','A64' }) do segregation_pool[#segregation_pool + 1] = v end
	end

	if pool_settings['is_lawful'] then
		for k,v in ipairs({ 'A58','A58','B59','B59' }) do segregation_pool[#segregation_pool + 1] = v end
	end
	...
	if pool_settings['has_strong_impact'] then
		for k,v in ipairs({ 'A40','A40','A40','A40','A40' }) do segregation_pool[#segregation_pool + 1] = v end

		if not pool_settings['has_heavy_stun'] then
			for k,v in ipairs({ 'A56','A56','A56','A56','A56' }) do segregation_pool[#segregation_pool + 1] = v end
		end
	end
	...
	if pool_settings['has_fire_attack'] and not pool_settings['has_area_attack'] then
		for k,v in ipairs({ 'C53','C53',  }) do segregation_pool[#segregation_pool + 1] = v end
	end

	if pool_settings['has_cold_attack'] and not pool_settings['has_strong_cold_ability'] then
...
local early_allowed_buffs_table = { 
	'A1','A3','A6','A7','A8','A9','A11','A12','A13','A15','A16','A18','A19','A22','A23','A24','A25','A28','A29','A31','A39','A47','A48','A49','A50','A51','A52','A53','A54','A57','A58','A59','A60','A61','A62','A63','A64',
...
function wesnoth.wml_actions.qquws_generate_random_champion(cfg)
	...
	local pool_a = { 'A1','A3','A5','A7','A8','A10','A11','A12','A16','A18','A19','A22','A23','A26','A29','A31','A32','A36','A63', }
	local pool_b = { 'B1','B10','B11','B12','B14','B15','B16','B18','B19','B21','B22','B24','B25','B29','B32','B35','B45','B46','B47','B48','B49','B50','B52','B56','B57','B58' }
	local pool_c = { 'C1','C2','C3','C4','C9','C13','C16','C17','C18','C21','C22','C29','C33','C35','C38','C43','C44','C45','C46','C50','C58','C59','C60','C62' }
	
	local pool_settings = {
		['human_player_restriction'] = wml.variables['qquws_champion_data.human_player_restriction'],
		...
		['is_lawful'] = wml.variables['qquws_champion_data.is_lawful'],
		...
		['has_heavy_stun'] = wml.variables['qquws_champion_data.has_heavy_stun'],
		['has_fire_attack'] = wml.variables['qquws_champion_data.has_fire_attack'],
		...
		['has_multiple_str_melee'] = wml.variables['qquws_champion_data.has_multiple_str_melee'],
		['has_multiple_str_ranged'] = wml.variables['qquws_champion_data.has_multiple_str_ranged'],
	}

	if pool_settings['human_player_restriction'] then
		for k,v in ipairs({ 'A53','A55','A63','B37','B40','B51','B53','B54','B57','B59','B60','B64','C36','C49','C50','C51','C52','C54','C55','C56','C57','C59','C60','C61','C63','C64' }) do exclude_from_pool[#exclude_from_pool + 1] = v end
	end
